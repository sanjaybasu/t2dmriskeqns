...
ui = navbarPage("RECODe",
                ...
                tabPanel("Risk Calculator",
                         ...
                         fluidPage(
                           ...
                           h5("Switch to:", a("version using Conventional/US units", href="https://sanjaybasu.shinyapps.io/recode/")),
                           ...
                           fluidRow(
                             ...
                             column(3,
                                    numericInput("totchol", label = "Total cholesterol (mmol/L)", value = 5.0),
                                    numericInput("hdlchol", label = "HDL cholesterol (mmol/L)", value = 1.0),
                             ...
                             column(3,numericInput("sercreat", label = "Serum creatinine (micromol/L)", value = 70.0),
                                    numericInput("uralbcreat", label = "Urine albumin/creatinine ratio (mg/mmol)", value = 5.0),
...
server = function(input, output) {
  ...
  neph_beta = reactive({round(100*(1-.973^exp(0 + 
                                                                                          ...
                                                                                          0.8608801402 * as.numeric(input$sercreat)*0.9/78.6 + 
                                                                                          -0.0011121510 * as.numeric(input$totchol)*183.2/4.7+ 
                                                                                          0.0062888253 * as.numeric(input$hdlchol)*41.8/1.1 +
                                                                                          0.0319895697 * as.numeric(input$anticoag) + 
                                                                                          0.1369126389 * as.numeric(input$hgba1c) +
                                                                                          0.0003615507*as.numeric(input$uralbcreat)*99.2/11.3
      -0.2269629)))
    })
  ...
  retin_beta = reactive({round(100*(1-.921^exp(0 + 
                                           ...
                                           -0.1676573729 * as.numeric(input$black) + 
                                             ...
                                             0.6946500975 * as.numeric(input$sercreat)*0.9/78.6 + 
                                           -0.0001676169 * as.numeric(input$totchol)*183.2/4.7 + 
                                             0.0054470159 * as.numeric(input$hdlchol)*41.8/1.1 +
                                             0.1449446673 * as.numeric(input$hgba1c) +
                                             0.0001991881*as.numeric(input$uralbcreat)*99.2/11.3
                                          -4.563441)))
  })
  ...
  neuro_beta = reactive({round(100*(1-0.87^exp(0 + 
                                          ...
                                          6.044e-01 * as.numeric(input$sercreat)*0.9/78.6 + 
  ...
  mi_beta = reactive({round(100*(1-0.85^exp(0 + 
                                              ...
                                              0.435500 * as.numeric(input$sercreat)*0.9/78.6 + 
                                              0.001929 * as.numeric(input$totchol)*183.2/4.7 + 
                                              -0.008370 * as.numeric(input$hdlchol)*41.8/1.1 +
                                              0.171600 * as.numeric(input$hgba1c) +
                                              0.000333 *as.numeric(input$uralbcreat)*99.2/11.3+
                                                -3.65)))
  })
  ...
  chf_beta = reactive({round(100*(1-0.96^exp(0 + 
                                               ...
                                               4.142e-04 * as.numeric(input$uralbcreat)*99.2/11.3 + 
                                               8.214e-01 * as.numeric(input$sercreat)*0.9/78.6 + 
                                               -1.358e-03 * as.numeric(input$totchol)*183.2/4.7 + 
                                               -1.758e-02 * as.numeric(input$hdlchol)*41.8/1.1 +
  ...
  death_beta = reactive({round(100*(1-0.93^exp(0 + 
                                                 ...
                                                 3.889e-04 * as.numeric(input$uralbcreat)*99.2/11.3 + 
                                                 3.597e-01 * as.numeric(input$sercreat)*0.9/78.6 + 
                                                 -9.478e-04 * as.numeric(input$totchol)*183.2/4.7 + 
                                                 -4.378e-03 * as.numeric(input$hdlchol)*41.8/1.1 +
  ...
  output$summary = renderDataTable({
    rows = c("Age, mean (SD), y", "Senior in age, >75 y (%)", "Women", "Black race", "Hispanic or Latino ethnic group", "Tobacco smoking, current", "Body mass index, mean (SD), kg/m2",
             ...
             "Platelet aggregate inhibitor use", "Daily aspirin use", "Hemoglobin A1c, mean (SD), %", "Total cholesterol, mean (SD), mmol/L", "Direct high-density lipoprotein cholesterol, mean (SD), mmol/L",
             "Low-density lipoprotein cholesterol, mean (SD), mmol/L", "Triglycerides, mean (SD), mmol/L", "Fasting plasma glucose, mean (SD), mmol/L", "Alanine aminotransferase, mean (SD), IU/dL",
             "Creatine phosphokinase, mean (SD), IU/L", "Serum potassium, mean (SD), mmol/L", "Serum creatinine, mean (SD), micromol/L", "Estimated glomerular filtration rate, mean (SD), mL/min/1.73m2",
             "Urine albumin, mean (SD), micromol/L", "Urine creatinine, mean (SD), micromol/L", "Urine albumin/creatinine ratio, mean (SD), mg/mmol")
    accord = c("62.8 (6.7)", "521 (5.4)", "3,662 (38.0)", "1,834 (19.0)", "678 (7.0)", "1,179 (12.2)", "32.2 (5.4)", "136.5 (17.1)",
               ...
               "303 (3.1)", "851 (8.8)", "466 (4.8)", "5,274 (54.7)", "8.3 (1.1)", "4.1 (1.1)", "1.1 (0.3)", "2.7 (0.9)", "2.2 (1.6)",
               "9.7 (3.1)", "27.5 (16.0)", "140.3 (130.2)", "4.5 (0.5)", "78.6 (17.7)", "90.9 (27.3)", "0.2 (0.6)", "11.3 (5.8)", "11.3 (40.8)")
    dppos = c("50.9 (8.0)", "0 (0)", "680 (66.8)", "244 (24.0)", "175 (17.2)", "52 (5.1)", "33.9 (5.9)", "123.7 (14.0)",
              ...
              "6.1 (0.7)", "5.1 (1.1)", "1.2 (0.3)", "2.6 (0.7)", "1.8 (2.9)", "6.4 (1.3)", "N/A", "N/A", "N/A", "70.7 (17.7)",
              "98.8 (15.7)", "0.2 (0.1)", "10.9 (6.5)", "N/A")
    lookahead = c("58.9 (6.7)", "31 (0.7)", "2,784 (58.5)", "776 (16.3)", "670 (14.1)", "202 (4.2)", "36.0 (5.9)", "129.0 (17.1)", "70.2 (9.5)",
                  ...
                  "7.3 (1.2)", "5.0 (1.0)", "1.1 (0.3)", "2.9 (0.8)", "N/A", "8.5 (2.5)", "N/A", "N/A", "N/A",
                  "70.7 (17.7)", "89.9 (16.1)", "0.1 (0.3)", "10.7 (5.9)", "4.9 (22.9)")
